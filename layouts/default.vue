    ...
    <header class="header">
      <div class="container header-inner header-grid">
        ...
        <nav class="nav nav-right" aria-label="Primary">
          ...
          <button class="icon-btn" aria-label="Theme" @click="cycleTheme">
            <component :is="iconComp" :size="18" aria-hidden="true" />
          </button>
          <UiSelect v-model="localeCodeModel" :options="localeOptions" />
          <UiButton variant="outline" href="mailto:vise@seventysete.com">{{
          ...
          <ul v-if="menuOpen" class="menu mobile-menu-pop" role="menu">
            <li role="none">
              <button role="menuitem" class="menu-item" @click="cycleTheme">
                <span
                  >{{ t("theme.auto") }}/{{ t("theme.light") }}/{{
                    t("theme.dark")
                  }}</span
                >
              </button>
            ...
            <li role="none">
              <a class="menu-item" href="mailto:vise@seventysete.com">{{
                t("actions.suggestTool")
              }}</a>
            </li>
          </ul>
...
const isAllToolsPage = computed(() => {
  ...
  return (
    p.endsWith("/todas-as-ferramentas") ||
    p.endsWith("/tools") ||
...
const _footerCats = computed(() => {
  const items = (reg.value as Array<{ category: string }> | null) || [];
  const present = new Set(items.map((i: { category: string }) => i.category));
...
onMounted(() => {
  try {
    ...
    const onDocClick = (ev: MouseEvent) => {
      ...
      if (!(ev.target as HTMLElement).closest(".mobile-menu-pop"))
