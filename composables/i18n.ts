...
export const SUPPORTED = ['en', 'pt-br', 'es'] as const
export type Locale = typeof SUPPORTED[number]

const messages: Record<Locale, Record<string, string>> = { en, 'pt-br': ptBR, es }

export function normalizeLocale(l?: string | null): Locale | null {
  if (!l) return null
  const low = l.toLowerCase()
  if (SUPPORTED.includes(low as Locale)) return low as Locale
...
export function detectLocale(): Locale {
  ...
  if (process.client && typeof navigator !== 'undefined') {
    ...
    for (const l of langs) { const n = normalizeLocale(l); if (n) return n }
  }
  return 'en'
}
