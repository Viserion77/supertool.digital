    ...
    <section aria-labelledby="featured">
      <div class="container">
        ...
        <ClientOnly>
          ...
          <template #fallback>
            <div class="grid mt-16">
              <div class="card skeleton-card" v-for="n in 6" :key="n"></div>
            </div>
          </template>
        </ClientOnly>
      </div>
...
const categoryLabel = (key: string) => key === 'conversores' ? t('nav.converters') : key === 'geradores' ? t('nav.generators') : key === 'ferramentas-web' ? t('nav.webTools') : key

const { data: registry } = await useFetch('/api/tools')
const toolsTotal = computed(() => ((registry.value as any[] | null)?.length || 0))
const toolsTotalDisplay = computed(() => `${Math.max(0, toolsTotal.value - 3)}+`)

const allTools = computed(() => {
  const loc = (useI18n().locale.value as string) || 'en'
  const items = (registry.value as any[] | null) || []
  return items.map((it) => {
    const badge = (it.badges && it.badges[0]) ? it.badges[0] : null
    const badgeType = badge?.type === 'popular' ? 'popular' : badge?.type === 'new' ? 'new' : null
    return {
...
const categories = computed(() => {
  ...
  const iconMap: Record<string, any> = { conversores: ArrowLeftRight, geradores: Zap, 'ferramentas-web': Globe }
  const variantMap: Record<string, 'blue' | 'green' | 'yellow'> = { conversores: 'blue', geradores: 'green', 'ferramentas-web': 'yellow' }

  return order
    .filter((k) => (groups[k] && groups[k].length > 0))
    .map((k) => {
      const tools = groups[k]
      const tags = tools.slice(0, 3).map((it) => it.title?.[loc] || it.title?.en || '')
...
const visibleTools = computed(() => {
  ...
  return list.filter(t => [t.title, t.description, ...(t.keywords||[])].join(' ').toLowerCase().includes(query))
})
