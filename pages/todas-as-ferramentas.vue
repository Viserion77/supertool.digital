<template>
  <section class="container" aria-labelledby="all-tools-title">
    ...
    <div class="toolbar-row mt-16">
      <input ref="qInputRef" class="header-input" v-model="q" type="search" placeholder="Buscar ferramentas..." aria-label="Buscar ferramentas" />
    </div>
...
onMounted(() => { if (q.value) focusInput() })
...
watch(() => route.query.cat, (val) => { cat.value = typeof val === 'string' ? val : '' })
...
const filtered = computed(() => {
  ...
  if (!term) return base
  return base.filter((t) => [t.title, t.description, ...(t.keywords||[])].join(' ').toLowerCase().includes(term))
})

definePageMeta({ alias: ['/pt-br/todas-as-ferramentas','/en/tools','/es/herramientas'] })
</script>
