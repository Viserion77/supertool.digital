<template>
  <canvas ref="canvas" class="particles-surface" aria-hidden="true"></canvas>
</template>
...
const draw = () => {
  ...
  for (const p of particles) {
    ...
    const MAX = 0.8
    if (spd > MAX) {
      const ang = Math.atan2(p.vy, p.vx) + Math.PI + (Math.random()-0.5)*0.4
...
onMounted(() => {
  ...
  onMove = (e: MouseEvent) => { const r = canvas.value?.getBoundingClientRect(); if (r) { mouse.x = e.clientX - r.left; mouse.y = e.clientY - r.top } }
  onLeave = () => {
...
onUnmounted(() => {
  ...
  onResize = null; onMove = null; onLeave = null; canvasEl = null
})
</script>
